	...
	public void map(ResultSet rs) throws UnexpectedInputException, NoSuchMethodException, SecurityException,
		...
		for (int i = 0; i < fields.length; i++) {
			if (Modifier.isTransient(fields[i].getModifiers())) {
				...
				String dbName = name.replaceAll("(.)([A-Z])", "$1_$2").toLowerCase();
				name = Character.toUpperCase(name.charAt(0)) + name.substring(1);
				String fieldType = fields[i].getType().toString().toLowerCase();
				if (fieldType.contains("int")) {
					Method method = cls.getDeclaredMethod("set" + name, new Class[] { Integer.TYPE });
					method.invoke(this, rs.getInt(dbName));
				} else if (fieldType.endsWith("double")) {
					Method method = cls.getDeclaredMethod("set" + name, new Class[] { Double.TYPE });
					method.invoke(this, rs.getDouble(dbName));
				} else if (fieldType.endsWith("long")) {
					Method method = cls.getDeclaredMethod("set" + name, new Class[] { Long.TYPE });
					method.invoke(this, rs.getLong(dbName));
				} else if (fieldType.endsWith("boolean")) {
					Method method = cls.getDeclaredMethod("set" + name, new Class[] { Boolean.TYPE });
					method.invoke(this, rs.getBoolean(dbName));
				} else if (fieldType.endsWith("string")) {
					Method method = cls.getDeclaredMethod("set" + name, new Class[] { String.class });
					method.invoke(this, rs.getString(dbName));
				} else if (fieldType.endsWith("timestamp")) {
					Method method = cls.getDeclaredMethod("set" + name, new Class[] { Timestamp.class });
					method.invoke(this, rs.getTimestamp(dbName));
				} else {
					throw new UnexpectedInputException("The generic mapper does not deal with: " + fieldType
							+ " add it to BaseDao");
				}
